...
A lightweight, stable OpenWRT DNS failover system. It detects default DNS resolution failures, and switches dnsmasq resolvers to a local DNS resolver (such as Xray/Passwall DNS) to prevent connectivity dropouts.

## What it DOES:
- **Instability Detection**: Measures DNS health against `1.1.1.1` and `8.8.8.8` directly.
...
- **Transitions**: Switches to `FAILOVER` after 2 consecutive failure cycles. Returns to `NORMAL` after 1 successful probe run.
